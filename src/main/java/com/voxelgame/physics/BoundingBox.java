...
public class BoundingBox {
    private float epsilon = 0.001f; // 小正值，避免微小浮动累积
    ...
    private float clipCollide(BoundingBox c, float a, int axis) {
        ...
        if (a > 0.0F) {
            float max = (axis == 0 ? this.x0 : axis == 1 ? this.y0 : this.z0) -
                        (axis == 0 ? c.x1 : axis == 1 ? c.y1 : c.z1) - epsilon;
            if (max < a) a = Math.max(0, max); // 阻止微小正值累积
        } else if (a < 0.0F) {
            float max = (axis == 0 ? this.x1 : axis == 1 ? this.y1 : this.z1) -
                        (axis == 0 ? c.x0 : axis == 1 ? c.y0 : c.z0) + epsilon;
            if (max > a) a = Math.min(0, max); // 阻止微小负值累积
        }
        return a;
    }
